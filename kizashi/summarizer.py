...
与えられた記事について、**記事を開かなくても内容が分かる**深掘り要約を書きます。

# 要約に必ず含める
- 何が発表/議論されたのか
- なぜ重要か + 具体的な数字 (性能・価格・規模など分かるもの)
- 開発者視点での影響 (実務にどう効くか)

# 制約
- 日本語で{smin}〜{smax}字。
- 本文の文をそのまま写さない。必ず自分の言葉で書き直す。
- 誇張しない。分からないことは断定しない。
- 本文が取得できていない場合は、タイトルとメタ情報から分かる範囲で要約し、
  末尾に「(本文取得できず、タイトル等からの推定を含む)」と明記する。
