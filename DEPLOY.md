...
上げるランプ(60分前50% / 15分前80% / 5分前90%)に当たるまで回す。**天井に当たっても即終了せず、
実行時間が残っていれば次の枠リセットまで待って再開する**(夜に自分の対話で枠を使い切った直後に
起動しても、リセット後に空く枠を拾えるようにするため)。終了条件は「実行時間切れ / プール空 /
件数上限 / 週次ガード / 実行時間内に次のリセットが来ない」。分母(プランの5h上限)は
...
**実行過程をチャットに残す**: `.env` に `DISCORD_WEBHOOK_URL`(Discord > チャンネル設定 >
連携サービス > Webフックで発行、ボット不要)を入れると、夜間ジョブが**開始 / 週次解析の本文 /
新ソース候補の本文 / バックフィル進捗(既定50件ごと)/ 終了サマリ**を Discord に投稿する。
翌朝スマホからスクロールで追える。LINE(ユーザー向けダイジェスト)とは別の運用ログ用の口。
未設定なら `run.log` のみ。無効化は `--no-chat`、進捗頻度は `--chat-every`。
